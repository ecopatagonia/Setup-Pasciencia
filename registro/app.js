(() => {
  ...
  const HOME_URL = "../";
  const SUCCESS_KEY = "pulo_gatto_registration_success";

  /*
   * Si la página de confirmación fue mostrada y el usuario actualiza
   * el navegador, lo enviamos al login.
   */
  ...
  const successWasShown =
    sessionStorage.getItem(SUCCESS_KEY) === "true";

  if (successWasShown && navigationType === "reload") {
  ...
  /*
   * Si volvió a entrar normalmente a registro, permitimos completar
   * un nuevo formulario en este dispositivo.
   */
  if (successWasShown && navigationType !== "reload") {
    sessionStorage.removeItem(SUCCESS_KEY);
  }
  ...
  function parsePhone() {
    ...
    const raw =
      `${ddiInput.value}` +
      `${digits(areaInput.value)}` +
      `${digits(numberInput.value)}`;
    ...
    return window.libphonenumber.parsePhoneNumberFromString(
      raw,
      iso2
    );
  }
  ...
  function validatePhone(showError = true) {
    ...
    if (valid) {
      preview.textContent =
        `${phone.formatInternational()} • pronto para WhatsApp`;
    ...
    preview.textContent =
      "Será salvo no formato internacional para uso no WhatsApp.";
  ...
  async function api(payload) {
    if (
      ...
      throw new Error(
        "A URL do Apps Script ainda não foi configurada."
      );
    }
  ...
  document
    .querySelector("#pin")
    .addEventListener("input", numericInput);

  document
    .querySelector("#pin-confirm")
    .addEventListener("input", numericInput);
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const pinConfirm =
      document.querySelector("#pin-confirm").value;
    ...
    if (!/^\d{4}$/.test(pin) || pin !== pinConfirm) {
      showMessage(
        "As duas chaves precisam ter os mesmos 4 números."
      );
