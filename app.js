...
const state = { all: [], filtered: [], period: 'all', stats: null, resultFilter: 'all', search: '' };
...
function sideStats(side) {
  ...
  const gains = ops.filter(op => op.resultado === 'GAIN').length;
  return { ops, total, gains, rate: ops.length ? gains / ops.length * 100 : 0, average: ops.length ? total / ops.length : 0 };
}
...
function renderSideComparison() {
  ...
  container.innerHTML = ['COMPRA', 'VENDA'].map(side => {
    ...
    return `<article class="comparison-card">
      ...
      <dl><div><dt>Operações</dt><dd>${s.ops.length}</dd></div><div><dt>Acerto</dt><dd>${number2.format(s.rate)}%</dd></div><div><dt>Expectativa</dt><dd>${money.format(s.average)}</dd></div></dl>
    </article>`;
  }).join('');
}
...
function renderHours() {
  ...
  list.innerHTML = rows.map(row => `<div class="hour-row">
    ...
    <small>${row.count} ops · ${number2.format(row.count ? row.gains / row.count * 100 : 0)}%</small>
  </div>`).join('');
}
