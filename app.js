...
function operationDurationMinutes(op) {
  const candles = Math.max(0, Number(op.candleSaida) - Number(op.candleEntrada));
  return candles * 5;
}
...
function renderHours() {
  ...
  if (profiles) profiles.innerHTML = profileRows.map(profile => {
    ...
    const duration = median(profile.ops.map(operationDurationMinutes));
    const result = median(profile.ops.map(op => Number(op.financeiro)));
    return `<article class="comparison-card"><div class="comparison-title"><span>${profile.label}</span><strong class="${profile.kind}">${profile.ops.length}</strong></div><dl><div><dt>Início mediano</dt><dd>${startLabel}</dd></div><div><dt>Duração mediana</dt><dd>${minutesLabel(duration)}</dd></div><div><dt>Resultado mediano</dt><dd class="${profile.kind}">${money.format(result)}</dd></div></dl></article>`;
  }).join('');
...
function drawMultiLineChart(canvas, series, labels, formatAxis) {
  if (!canvas || !labels.length) return;
  const {ctx,width,height}=setupCanvas(canvas);
  const pad={top:24,right:20,bottom:58,left:72};
...
function drawHourDurationHeatmap() {
  ...
  cells.forEach(cell => {
    ...
    if (cell.ops.length) {
      ctx.fillStyle = cell.value >= 0 ? '#061417' : '#fff';
...
function drawAverages() {
  ...
  ],labels,value=>money0.format(value));
}
