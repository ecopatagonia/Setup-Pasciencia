...
async function loadData() {
  ...
  try {
    ...
    if (state.all.length) {
      ...
      document.getElementById('customFrom').value = state.customFrom;
      document.getElementById('customTo').value = state.customTo;
    }
...
['customFrom', 'customTo'].forEach(id => document.getElementById(id)?.addEventListener('change', event => {
  if (id === 'customFrom') state.customFrom = event.target.value;
  else state.customTo = event.target.value;
  state.period = 'custom';
  applyPeriod();
}));
document.querySelectorAll('#analysisMode button').forEach(button => {
...
function setTheme(theme) {
  ...
  localStorage.setItem('pulo-theme', theme);
...
setTheme(localStorage.getItem('pulo-theme') === 'light' ? 'light' : 'dark');
