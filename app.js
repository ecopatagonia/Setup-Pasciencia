...
function renderRisk(dayValues) {
  ...
  const dailyLimitValue = chosenPoints * riskConfig.contracts * POINT_VALUE;
  const exceedanceCount = pointDays.filter(value => chosenPoints > 0 && value <= -chosenPoints).length;
  const exceedanceRate = pointDays.length ? exceedanceCount / pointDays.length * 100 : 0;
  const worstLossRun = pointLosses.reduce((worst, run) => Math.abs(run.sum) > Math.abs(worst.sum) ? run : worst, { sum: 0, length: 0 });
  setText('projection-exceedance', `${exceedanceCount} de ${pointDays.length}`);
  setText('projection-exceedance-note', `${percent2(exceedanceRate)} dos pregões alcançaram ou superaram o limite escolhido.`);
  setText('projection-technical-limits', `${number2.format(dailyLimitValue ? technicalCapital / dailyLimitValue : 0)} ×`);
  setText('projection-historical-limits', `${number2.format(dailyLimitValue ? statisticalCapital / dailyLimitValue : 0)} ×`);
  setText('projection-worst-sequence', money.format(-Math.abs(worstLossRun.sum) * riskConfig.contracts * POINT_VALUE));
  setText('projection-worst-sequence-note', `${worstLossRun.length} ${worstLossRun.length === 1 ? 'pregão negativo consecutivo' : 'pregões negativos consecutivos'}.`);
  const projectionConfig = document.getElementById('projectionConfig');
  if (projectionConfig) projectionConfig.textContent = `${riskConfig.contracts} ${riskConfig.contracts === 1 ? 'contrato' : 'contratos'} · limite diário de ${points0.format(chosenPoints)} pontos · máximo de ${riskConfig.maxOperations} operações`;
}
...
function alignedRange(values, base = 50, targetTicks = 8) {
  const min = Math.min(0, ...values), max = Math.max(0, ...values);
  const raw = Math.max(base, (max - min) / targetTicks);
  const magnitude = 10 ** Math.floor(Math.log10(raw));
  const normalized = raw / magnitude;
  const nice = normalized <= 1 ? 1 : normalized <= 2 ? 2 : normalized <= 2.5 ? 2.5 : normalized <= 5 ? 5 : 10;
  const step = Math.max(base, Math.ceil((nice * magnitude) / base) * base);
  return {
...
function drawLineChart(canvas, values, options = {}) {
  ...
  const tickValues = options.tickStep
    ? Array.from(
        { length: Math.round((range.max - range.min) / options.tickStep) + 1 },
        (_, index) => range.max - index * options.tickStep
      )
    : Array.from({ length: 5 }, (_, index) => range.max - (range.max - range.min) * index / 4);
  ...
  if (zeroY >= pad.top && zeroY <= pad.top + plotH) {
    ctx.strokeStyle = options.zeroColor || (options.prominentZero ? '#ff5164' : (values.some(v => v < 0) ? '#ff5164' : 'rgba(139,154,161,.55)'));
  ...
  (options.referenceLines || []).forEach(line => {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(line.label, pad.left + 8, Math.max(pad.top + 9, yy - 10));
  });
...
function drawPatrimony() {
  ...
  const values = state.stats.curve.map(p => p.value);
  const rawStep = capital > 0 ? capital / 8 : Math.max(50, (Math.max(...values, 1) - Math.min(...values, 0)) / 8);
  const step = Math.max(50, Math.ceil(rawStep / 50) * 50);
  const minValue = Math.min(-capital, ...values, 0);
  ...
  drawLineChart(document.getElementById('patrimonyChart'), values, {
    ...
    prominentZero: true,
    zeroColor: '#ffffff',
    referenceLines: capital > 0 ? [{ value: -capital, color: '#ff5164', width: 2.4, label: `−100% · ${money0.format(-capital)}` }] : [],
    formatAxis: value => money0.format(value),
    formatMarker: value => capital > 0
      ? `${percent2(value / capital * 100)} · ${money.format(value)}`
      : money.format(value)
  });
}
...
function drawBarChart(canvas, rows, formatAxis, options = {}) {
  ...
  const aligned = options.tickBase ? alignedRange(values, options.tickBase) : null;
  const range = aligned || niceRange(values);
  ...
  const ticks = aligned
    ? Array.from({ length: Math.round((range.max - range.min) / aligned.step) + 1 }, (_, i) => range.max - i * aligned.step)
    : Array.from({ length: 5 }, (_, i) => range.max - (range.max - range.min) * i / 4);
  ticks.forEach(value => {
    const yy = y(value);
...
function drawAll() {
  ...
  const factor = Number(document.getElementById('safetyFactor')?.value) || 3;
  const stopPoints = Math.max(0, numericInputValue('technicalStop'));
  const technicalStart = stopPoints * riskConfig.maxOperations * riskConfig.contracts * POINT_VALUE * factor;
  const pointDays = groupByDay(state.filtered.map(op => ({...op, financeiro: Number(op.pontos)}))).map(day => day.value);
  const dayValues = pointDays.map(value => value * riskConfig.contracts * POINT_VALUE);
  const pointLosses = streakRuns(pointDays, value => value < 0);
  const historicalRiskPoints = Math.max(calculateCurve(pointDays).drawdown, Math.abs(Math.min(0, ...pointLosses.map(run => run.sum))));
  const historicalStart = historicalRiskPoints * riskConfig.contracts * POINT_VALUE * factor;
  const accumulateFrom = start => {
    let current = start;
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      ...
      if (loadingDetail) {
        loadingDetail.textContent = attempt === 1
          ...
          : `A conexão demorou. Nova tentativa ${attempt} de 2…`;
      }
      try {
...
document.querySelectorAll('.nav-button[data-page]').forEach(button => {
  button.addEventListener('click', () => {
    ...
    document.querySelectorAll('.nav-group').forEach(group => group.classList.toggle('open', group.contains(button)));
    document.querySelectorAll('.nav-group-toggle').forEach(toggle => toggle.setAttribute('aria-expanded', String(toggle.parentElement.classList.contains('open'))));
...
document.querySelectorAll('.nav-group-toggle').forEach(toggle => {
  toggle.addEventListener('click', () => {
    ...
    document.querySelectorAll('.nav-group-toggle').forEach(button => button.setAttribute('aria-expanded', String(button.parentElement.classList.contains('open'))));
  });
});
