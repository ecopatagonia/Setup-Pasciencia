...
const state = { all: [], filtered: [], period: 'all', mode: 'operations', stats: null, resultFilter: 'all', search: '' };
...
function render() {
  ...
  const periodNames = { all: 'Todo o período', last: 'Último dia carregado', week: 'Semana da última data', month: 'Mês da última data', year: 'Ano da última data' };
...
function renderRisk(dayValues) {
  ...
  setText('risk-worst-operation', `${number2.format(worstOperationPoints)} pts`);
  setText('risk-worst-day-points', `${number2.format(worstDayPoints)} pts`);
  setText('risk-drawdown-points', `${number2.format(-pointStats.drawdown)} pts`);
  ...
  setText('risk-technical-points', `${number2.format(technicalDailyPoints)} pts`);
  setText('risk-technical-value', money.format(technicalDailyValue));
  setText('risk-technical-formula', `${number2.format(stopPoints)} pts × ${riskConfig.contracts} ${riskConfig.contracts===1?'contrato':'contratos'} × ${riskConfig.maxOperations} operações`);
...
function renderOperations() {
  ...
  if (state.mode === 'days') {
    const rows = groupByDay(state.filtered).reverse().filter(day => {
      ...
      return resultOk && (!search || formatDate(day.date).toLowerCase().includes(search));
    });
    head.innerHTML = '<th>Data</th><th>Operações</th><th>Resultado do dia</th><th>Financeiro</th>';
    body.innerHTML = rows.map(day => {
      const result = day.value > 0 ? 'GAIN' : day.value < 0 ? 'STOP' : 'BREAKEVEN';
      return `<tr><td>${formatDate(day.date)}</td><td>${day.count}</td><td><span class="result-badge ${result.toLowerCase()}">${result}</span></td><td class="${day.value >= 0 ? 'positive' : 'negative'}">${money.format(day.value)}</td></tr>`;
    }).join('');
