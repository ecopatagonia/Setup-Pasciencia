...
function renderRisk(dayValues) {
  ...
  setText('risk-days', capital>0&&dailyLimit>0 ? number2.format(capital/dailyLimit) : '—');
  setText('risk-dd-percent', capital>0 ? percent2(-dayStats.drawdown/capital*100) : '—');
  setText('risk-reserve', capital>0 ? money.format(capital+worstSequence) : '—');
}
...
function updatePatrimony() {
  ...
  const capital = Math.max(1, capitalValue);
  const p = value => value / capital * 100;
  const s = state.stats;

  const display = value => capitalValue ? percent2(value) : '—';
...
document.querySelectorAll('#periodSelector button').forEach(button => {
  button.addEventListener('click', () => {
    ...
    document.querySelectorAll('#periodSelector button').forEach(item => item.classList.toggle('active', item === button));
...
document.querySelectorAll('#analysisMode button').forEach(button => {
  button.addEventListener('click', () => {
    ...
    document.querySelectorAll('#analysisMode button').forEach(item => item.classList.toggle('active', item === button));
