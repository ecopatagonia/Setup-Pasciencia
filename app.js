...
function renderExtended() {
  ...
  if (streakEl) {
    ...
    streakEl.innerHTML = `<article class="comparison-card"><div class="comparison-title"><span>GAINS</span><strong class="positive">${maxGain} consecutivos</strong></div><p>${gainRuns.length} sequências positivas em ${unit}.</p></article><article class="comparison-card"><div class="comparison-title"><span>LOSSES</span><strong class="negative">${maxLoss} consecutivos</strong></div><p>${lossRuns.length} sequências negativas em ${unit} · pior impacto ${money.format(worstRun.sum)}.</p></article>`;
  }
  ...
  if (concentration) {
    ...
    concentration.innerHTML = `<article class="comparison-card"><div class="comparison-title"><span>GANHOS</span><strong class="positive">${pos.length} dias</strong></div><dl><div><dt>Ganho bruto</dt><dd>${money0.format(grossPos)}</dd></div><div><dt>Melhor dia</dt><dd>${money0.format(pos[0]?.value||0)}</dd></div><div><dt>Top 3</dt><dd>${number2.format(grossPos ? sum(pos,3)/grossPos*100:0)}%</dd></div><div><dt>Sem top 3</dt><dd class="${withoutBest < 0 ? 'negative' : ''}">${money0.format(withoutBest)}</dd></div></dl></article><article class="comparison-card"><div class="comparison-title"><span>PERDAS</span><strong class="negative">${neg.length} dias</strong></div><dl><div><dt>Perda bruta</dt><dd class="negative">${money0.format(-grossNeg)}</dd></div><div><dt>Pior dia</dt><dd class="negative">${money0.format(neg[0]?.value||0)}</dd></div><div><dt>Top 3</dt><dd>${number2.format(grossNeg ? sum(neg,3)/grossNeg*100:0)}%</dd></div><div><dt>Sem piores 3</dt><dd class="${withoutWorst < 0 ? 'negative' : ''}">${money0.format(withoutWorst)}</dd></div></dl></article>`;
  }
...
function drawBarChart(canvas, rows, formatAxis) {
  ...
  rows.forEach((row, i) => {
    ...
    if (rows.length <= 18 || i % Math.ceil(rows.length / 12) === 0) {
      ...
      ctx.fillStyle = cssColor('--muted'); ctx.textAlign = 'right'; ctx.fillText(row.label, 0, 0); ctx.restore();
    }
  });
  if (rows.length > 1) {
...
function drawAll() {
  ...
  const pointDays = groupByDay(state.filtered.map(op => ({...op, financeiro: Number(op.pontos)}))).map(day => day.value);
  const historicalStart = Math.max(calculateCurve(pointDays).drawdown, Math.abs(Math.min(0, ...pointDays))) * riskConfig.contracts * POINT_VALUE * factor;
