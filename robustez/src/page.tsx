...
type SavedScenarioConfig =
  | { kind: "Histórico"; granularity: PeriodGranularity; mode: LegacyExclusionMode; percent: number; positions: number[]; manualSelected: string[] }
...
function operationCurveForHistoricalScenario(granularity: PeriodGranularity, removed: Set<string>, positions: number[]) {
  const dayLabels = new Map(REAL_DAYS.map((day) => [day.date, day.label]));
  const operations = REAL_OPERATIONS
...
function Historicos({ riskCapital, setRiskCapital, contracts, setContracts, riskPoints, unit, setUnit, savedScenarios, setSavedScenarios }: { riskCapital: number; setRiskCapital: (value: number) => void; contracts: number; setContracts: (value: number) => void; riskPoints: number; unit: Unit; setUnit: (unit: Unit) => void; savedScenarios: SavedScenario[]; setSavedScenarios: (items: SavedScenario[]) => void }) {
  ...
  const realOperationStats = useMemo(() => statsWithCurve(realStats, cumulativeCurve(REAL_OPERATIONS.map((operation) => ({ label: `${operation.label} · ${operation.position}ª`, points: operation.points }))), riskPoints), [realStats, riskPoints]);
  ...
  const scenarioOperationStats = useMemo(() => statsWithCurve(scenarioResult, operationCurveForHistoricalScenario(granularity, scenario.removed, positions), riskPoints), [scenarioResult, granularity, scenario.removed, positions, riskPoints]);
