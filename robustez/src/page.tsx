...
import { DATA_AUDIT, REAL_DAYS, REAL_OPERATIONS, type RealDay, type RealOperation } from "./robustez-runtime-data";

type Tab = "Resumo de Robustez" | "Cenários Históricos" | "Stress Determinístico";
type Unit = "points" | "money" | "percent";
type RiskLevel = "Baixo" | "Médio" | "Alto" | "Crítico";
type ScenarioKind = "Histórico" | "Stress";
type PeriodGranularity = "Dias" | "Semanas" | "Meses";
type ExclusionMode = "Manual" | "Melhores" | "Aleatório positivo";
type LegacyExclusionMode = ExclusionMode | "Aleatório";
type DrawdownScope = "days" | "operations";
type TemporalMode = "Todas" | "Por ano" | "Personalizado";
...
type SavedScenarioConfig =
  | { kind: "Histórico"; granularity: PeriodGranularity; mode: LegacyExclusionMode; percent: number; positions: number[]; manualSelected: string[]; temporalMode?: TemporalMode; year?: string; from?: string; to?: string }
...
const TABS: Tab[] = ["Resumo de Robustez", "Cenários Históricos", "Stress Determinístico"];
...
function statsWithCurve(baseStats: ScenarioStats, curve: CurvePoint[], riskPoints: number): ScenarioStats {
  const minCurve = Math.min(...curve.map((point) => point.value), 0);
  const touchesRisk = curve.some((point) => point.value <= -riskPoints);
  const marginPct = riskPoints ? Math.max(0, Math.min(100, ((minCurve + riskPoints) / riskPoints) * 100)) : 100;
  const dd = maxDrawdownFromCurve(curve);
  return { ...baseStats, curve, dd, up: maxUpdownFromCurve(curve), minCurve, touchesRisk, marginPct, risk: riskFromMargin(touchesRisk, marginPct, dd.value, riskPoints) };
}
...
function daysByOperationPositions(positions: number[], sourceDays: ScenarioDay[], sourceOperations: RealOperation[]): ScenarioDay[] {
  const byDate = sourceOperations.reduce<Record<string, RealOperation[]>>((acc, operation) => { (acc[operation.date] ??= []).push(operation); return acc; }, {});
  return sourceDays.map((day) => { const ops = (byDate[day.date] ?? []).filter((operation) => positions.includes(operation.position)); return { date: day.date, label: day.label, points: ops.reduce((sum, operation) => sum + operation.points, 0), operations: ops.length, gains: ops.filter((operation) => operation.result === "GAIN").length, losses: ops.filter((operation) => operation.result === "LOSS").length, breakevens: ops.filter((operation) => operation.result === "BREAKEVEN").length, zeroBySelection: ops.length === 0 }; });
}
...
function periodKeyFromDate(date: string, granularity: PeriodGranularity) {
  if (granularity === "Semanas") return weekKey(date);
  if (granularity === "Meses") return monthKey(date);
  return date;
}
...
function normalizeExclusionMode(mode: LegacyExclusionMode): ExclusionMode {
  return mode === "Aleatório" ? "Aleatório positivo" : mode;
}
function buildHistoricalScenario(baseDays: ScenarioDay[], granularity: PeriodGranularity, mode: ExclusionMode, percent: number, manualSelected: Set<string>) {
  ...
  if (mode === "Manual") removed = new Set(manualSelected);
    ...
    const positivePeriods = periodTotals.filter((period) => period.points > 0);
    removed = new Set(shuffle(positivePeriods, rand).slice(0, Math.min(quantity, positivePeriods.length)).map((period) => period.key));
  }
  return { removed, days: baseDays.filter((day) => !removed.has(periodKey(day, granularity))), totalPeriods: periodTotals.length, quantity };
}
function operationCurveForHistoricalScenario(granularity: PeriodGranularity, removed: Set<string>, positions: number[], sourceDays: ScenarioDay[], sourceOperations: RealOperation[]) {
  const dayLabels = new Map(sourceDays.map((day) => [day.date, day.label]));
  const operations = sourceOperations
    .filter((operation) => positions.includes(operation.position))
    .filter((operation) => !removed.has(periodKeyFromDate(operation.date, granularity)))
    .map((operation) => ({ label: `${dayLabels.get(operation.date) ?? operation.date} · ${operation.position}ª`, points: operation.points }));
  return cumulativeCurve(operations);
}
...
function DrawdownMiniChart({ stats, unit, riskCapital, contracts, scope = "days", onScopeChange }: { stats: ScenarioStats; unit: Unit; riskCapital: number; contracts: number; scope?: DrawdownScope; onScopeChange?: (scope: DrawdownScope) => void }) {
  const segment = worstDrawdownSegment(stats.curve);
  const riskPoints = riskCapital / (0.2 * contracts);
  const durationUnit = scope === "operations" ? "operação(ões)" : "dia(s)";
  const width = 360;
  const height = 142;
  ...
  return <figure className="drawdown-mini"><div className="drawdown-summary"><div>{onScopeChange ? <><span>Curva DRAWDOWN</span><div className="drawdown-mode" role="group" aria-label="Selecionar curva de drawdown"><button className={scope === "days" ? "active" : ""} onClick={() => onScopeChange("days")} type="button">Por dia</button><button className={scope === "operations" ? "active" : ""} onClick={() => onScopeChange("operations")} type="button">Por operação</button></div></> : <span>Valor do DRAWDOWN</span>}</div><strong>{formatValue(segment.value, unit, riskCapital, contracts)}</strong><small>Duração: {segment.duration} {durationUnit}, de {segment.peakLabel} até {segment.endLabel}</small></div><svg aria-label="Pior DRAWDOWN observado" role="img" viewBox={`0 0 ${width} ${height}`}>
    <rect height={height} rx="8" width={width} x="0" y="0" />
    <line className="zero-line" x1={padding.left} x2={width - padding.right} y1={y(0)} y2={y(0)} />
...
function StandardMetricsPanel({ title, stats, realStats, unit, riskCapital, contracts, drawdownStats, drawdownRealStats, drawdownScope, setDrawdownScope }: { title: string; stats: ScenarioStats; realStats: ScenarioStats; unit: Unit; riskCapital: number; contracts: number; drawdownStats?: ScenarioStats; drawdownRealStats?: ScenarioStats; drawdownScope?: DrawdownScope; setDrawdownScope?: (scope: DrawdownScope) => void }) {
  const ddStats = drawdownStats ?? stats;
  const ddReal = drawdownRealStats ?? realStats;
  return <div className="panel scenario-metrics"><div className="section-title"><div><p>Métricas padrão</p><h2>{title}</h2></div><Help><p>Estas métricas se repetem nas telas de simulação para manter a mesma leitura: resultado, DRAWDOWN, margem contra capital e toque da linha vermelha.</p></Help></div><div className="compact-metrics"><MetricCard label="Resultado" tone={stats.total >= realStats.total ? "good" : "warn"} value={formatValue(stats.total, unit, riskCapital, contracts)} /><MetricCard label="Diferença vs real" tone={diffTone(stats.total - realStats.total)} value={formatValue(stats.total - realStats.total, unit, riskCapital, contracts)} /><MetricCard label="Pior DD" tone={ddStats.dd.value <= ddReal.dd.value ? "good" : "bad"} value={formatValue(ddStats.dd.value, unit, riskCapital, contracts)} /><MetricCard label="Margem capital" tone={stats.marginPct > 55 ? "good" : stats.marginPct > 25 ? "warn" : "bad"} value={pct(stats.marginPct)} /><MetricCard label="Linha vermelha" tone={stats.touchesRisk ? "bad" : "good"} value={stats.touchesRisk ? "Tocou" : "Não tocou"} /><MetricCard label="Risco" tone={stats.risk === "Baixo" ? "good" : stats.risk === "Crítico" ? "bad" : "warn"} value={stats.risk} /></div><DrawdownMiniChart contracts={contracts} onScopeChange={setDrawdownScope} riskCapital={riskCapital} scope={drawdownScope} stats={ddStats} unit={unit} /></div>;
}
...
function DataTable({ days }: { days: RealDay[] }) {
  return <div className="table-wrap"><table><thead><tr><th>Dia</th><th>Pts dia</th><th>Ops</th><th>GAIN</th><th>LOSS</th><th>BE</th></tr></thead><tbody>{days.slice(-10).reverse().map((day) => <tr key={day.date}><td>{day.label}</td><td className={day.points >= 0 ? "good-text" : "bad-text"}>{formatPts(day.points)}</td><td>{day.operations}</td><td>{day.gains}</td><td>{day.losses}</td><td>{day.breakevens}</td></tr>)}</tbody></table></div>;
}
...
function Historicos({ riskCapital, setRiskCapital, contracts, setContracts, riskPoints, unit, setUnit, savedScenarios, setSavedScenarios }: { riskCapital: number; setRiskCapital: (value: number) => void; contracts: number; setContracts: (value: number) => void; riskPoints: number; unit: Unit; setUnit: (unit: Unit) => void; savedScenarios: SavedScenario[]; setSavedScenarios: (items: SavedScenario[]) => void }) {
  const allDays = useMemo(() => baseDaysFromReal(), []);
  const years = useMemo(() => [...new Set(allDays.map((day) => day.date.slice(0, 4)))].sort(), [allDays]);
  const firstAvailable = allDays[0]?.date ?? "", lastAvailable = allDays.at(-1)?.date ?? "";
  const [temporalMode, setTemporalMode] = useState<TemporalMode>("Todas");
  const [selectedYear, setSelectedYear] = useState(years.at(-1) ?? "");
  const [customFrom, setCustomFrom] = useState(firstAvailable);
  const [customTo, setCustomTo] = useState(lastAvailable);
  ...
  const [drawdownScope, setDrawdownScope] = useState<DrawdownScope>("days");
  ...
  const displayedDrawdownStats = drawdownScope === "operations" ? scenarioOperationStats : scenarioResult;
  ...
  const activate = (item: SavedScenario) => {
    ...
    setMode(normalizeExclusionMode(item.config.mode));
    setPercent(item.config.percent);
    setPositions(item.config.positions);
    setManualSelected(new Set(item.config.manualSelected));
    setTemporalMode(item.config.temporalMode ?? "Todas"); setSelectedYear(item.config.year ?? years.at(-1) ?? ""); setCustomFrom(item.config.from ?? firstAvailable); setCustomTo(item.config.to ?? lastAvailable);
    setSavedMessage("Cenário ativado");
  };

  return <section className="panel-grid"><div className="panel wide temporal-base"><div className="section-title"><div><p>Base temporal do estudo</p><h2>Janela das comparações históricas</h2></div><Help><p>A referência histórica e o cenário aplicado usam sempre a mesma janela.</p></Help></div><div className="temporal-controls"><div className="segmented">{(["Todas", "Por ano", "Personalizado"] as TemporalMode[]).map((item) => <button className={temporalMode === item ? "active" : ""} key={item} onClick={() => { setTemporalMode(item); resetWindow(); }}>{item}</button>)}</div>{temporalMode === "Por ano" ? <label>Ano<select value={selectedYear} onChange={(event) => { setSelectedYear(event.target.value); resetWindow(); }}>{years.map((year) => <option key={year}>{year}</option>)}</select></label> : null}{temporalMode === "Personalizado" ? <div className="custom-period"><label>Desde<input max={lastAvailable} min={firstAvailable} type="date" value={customFrom} onChange={(event) => { setCustomFrom(event.target.value); resetWindow(); }} /></label><label>Até<input max={lastAvailable} min={firstAvailable} type="date" value={customTo} onChange={(event) => { setCustomTo(event.target.value); resetWindow(); }} /></label></div> : null}</div>{invalidRange ? <p className="warning">A data “Desde” não pode ser posterior à data “Até”.</p> : null}<p className={temporalDays.length ? "sample-caption" : "warning"}>{observedWindow}</p></div>{temporalDays.length ? <><div className="panel side"><div className="section-title"><div><p>Cenários Históricos</p><h2>Construir cenário</h2></div><Help><p>Manual começa neutro e reproduz o histórico real. Melhores remove os blocos mais positivos. Aleatório positivo retira apenas blocos com resultado positivo ao acaso para testar dependência dos ganhos.</p></Help></div><div className="segmented">{(["Dias", "Semanas", "Meses"] as PeriodGranularity[]).map((item) => <button className={granularity === item ? "active" : ""} key={item} onClick={() => { setGranularity(item); setManualSelected(new Set()); setSavedMessage(""); }}>{item}</button>)}</div><label>Excluir</label><select value={mode} onChange={(event) => { setMode(event.target.value as ExclusionMode); setSavedMessage(""); }}><option>Manual</option><option>Melhores</option><option>Aleatório positivo</option></select><label>Percentual da amostra: {percent}%</label><input max="30" min="0" step="1" type="range" value={percent} onChange={(event) => { setPercent(Number(event.target.value)); setSavedMessage(""); }} disabled={mode === "Manual"} /><p className="sample-caption">Amostra: {scenario.totalPeriods} {granularity.toLowerCase()}. Percentual escolhido exclui até {scenario.quantity} bloco(s){mode === "Aleatório positivo" ? " positivos" : ""}, arredondado para baixo.</p><label>Operações consideradas por dia</label><div className="check-row">{[1, 2, 3].map((position) => <label key={position}><input checked={positions.includes(position)} type="checkbox" onChange={() => { togglePosition(position); setSavedMessage(""); }} />{position}ª operação</label>)}</div><ActionButtons onApply={() => setSavedMessage("Cenário aplicado")} onClear={clear} onSave={save} saveLabel="Salvar cenário" savedMessage={savedMessage} /><RiskControls compact contracts={contracts} riskCapital={riskCapital} setContracts={setContracts} setRiskCapital={setRiskCapital} setUnit={setUnit} unit={unit} /></div><StandardMetricsPanel contracts={contracts} drawdownRealStats={displayedRealDrawdownStats} drawdownScope={drawdownScope} drawdownStats={displayedDrawdownStats} realStats={realStats} riskCapital={riskCapital} setDrawdownScope={setDrawdownScope} stats={scenarioResult} title="Cenário aplicado" unit={unit} /><div className="panel wide"><CurveChart contracts={contracts} riskCapital={riskCapital} riskPoints={riskPoints} series={[{ name: "Histórico real da janela", color: "#f8fafc", points: realStats.curve }, { name: "Cenário aplicado", color: RISK_COLORS[scenarioResult.risk], dashed: true, points: scenarioResult.curve }]} unit={unit} /></div><div className="panel"><div className="section-title"><div><p>Resumo da amostra</p><h2>Histórico da janela vs cenário aplicado</h2></div><Help><p>A referência e o cenário usam exatamente as mesmas operações da janela temporal selecionada.</p></Help></div><CompareTable contracts={contracts} real={realStats} riskCapital={riskCapital} scenario={scenarioResult} scenarioLabel="Cenário aplicado" unit={unit} /></div><div className="panel"><div className="section-title"><div><p>Leitura automática</p><h2>Impacto histórico</h2></div></div><p className="conclusion">O cenário remove {scenario.removed.size} bloco(s) em {granularity.toLowerCase()} dentro da janela selecionada. Resultado muda {formatValue(scenarioResult.total - realStats.total, unit, riskCapital, contracts)}; margem mínima contra capital fica em {pct(scenarioResult.marginPct)}.</p><ReadingGuide><p>As barras apagadas são períodos retirados do cenário. A linha branca representa a referência histórica da mesma janela temporal. O mini gráfico de DRAWDOWN pode alternar entre leitura por dia e por operação no mesmo recorte.</p></ReadingGuide></div><div className="panel wide"><div className="section-title"><div><p>Cenários salvos</p><h2>Ativar calibragem ou enviar ao Resumo</h2></div><Help><p>Máximo de 10 cenários salvos no módulo. Ao salvar o 11º, o mais antigo sai da lista.</p></Help></div><ScenarioList onActivate={activate} savedScenarios={savedOnly} setSavedScenarios={(items) => setSavedScenarios([...items, ...savedScenarios.filter((item) => item.kind !== "Histórico")].slice(0, 10))} /></div><div className="panel wide"><div className="section-title"><div><p>Barras de seleção</p><h2>{granularity} do histórico válido</h2></div><Help><p>Em Manual, clique nas barras para retirar ou recolocar dias, semanas ou meses. Em Melhores e Aleatório positivo, as barras removidas mostram a exclusão automática.</p></Help></div><PeriodBars bars={bars} granularity={granularity} onToggle={(key) => { setMode("Manual"); setSavedMessage(""); setManualSelected((current) => { const next = new Set(current); if (next.has(key)) next.delete(key); else next.add(key); return next; }); }} /></div></> : null}</section>;
}
...
function makeInitialSavedScenarios(): SavedScenario[] {
  try {
    ...
    return Array.isArray(parsed) ? parsed.filter((item) => item?.kind === "Histórico" || item?.kind === "Stress").slice(0, 10) : [];
  } catch {
    return [];
  }
}
